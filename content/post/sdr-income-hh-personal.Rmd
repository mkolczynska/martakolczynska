...
master_survey <- read.csv("C:/Users/mkolc/Google Drive/Academia/Blog/martakolczynska/content/post/data/sdr-income-master-survey.csv", 
                    stringsAsFactors = FALSE)

master_survey_gender <- read.csv("C:/Users/mkolc/Google Drive/Academia/Blog/martakolczynska/content/post/data/sdr-income-master-survey-gender.csv", 
                    stringsAsFactors = FALSE)

master1 <- read.csv("C:/Users/mkolc/Google Drive/Academia/Blog/martakolczynska/content/post/data/sdr-income-master.csv", 
                    stringsAsFactors = FALSE)
...
Of the 22 international survey projects in the SDR v.1.0 dataset, source variables measuring household income are available in 18 projects, and individual income in 10 projects. Surveys in 9 projects have both variables.
...
We created four harmonized target variables for household and personal income each. The first one is a cleaned version of the source variables with the original codes for substantive answers and missing value codes for non-responses (`T_INCOME_HH` and `T_INCOME_PERSONAL`). The second variant (`T_INCOME_HH_RANK_100` and `T_INCOME_PERSONAL_RANK_100`) represents the rank position of the respondent, rescaled into the range between 0 (lowest income) to 100 (highest income). The third variant rescales income to the scale 0-100 maintaining the relative distances between values (`T_INCOME_HH_PROP_100` and `T_INCOME_PERSONAL_PROP_100`). The fourth variant corresponds to mid-points from the cumulative distribution within the given national sample and represents the individual's relative position in the sample (`T_INCOME_HH_DISTRIB` and `T_INCOME_PERSONAL_DISTRIB`).
...
master_survey %>% ungroup() %>% select(t_survey_name, persinc_scale_length, hhinc_scale_length) %>%
  ...
  ylab("") + xlab("Numer of different realized response options (ln)") +
  ggtitle("Length of response scales to income variables across survey projects") +
...
master1 %>% filter(t_country_l1u == "PL" & !is.na(t_income_hh_prop_100)) %>%
  ungroup() %>%
