...
### Polyarchy: Semicolon delimited .csv
...
Country codes are available in the [Correlates of War format](http://www.correlatesofwar.org/data-sets/cow-country-codes){target="_blank"}, which needs converting to ISO3c with the `countrycode` function. The year and the participation variable we are primarily interested in also need converting to numeric.
...
### The Standardized World Income Inequality Database (SWIID): Plain .csv

Data from the [Standardized World Income Inequality Database (SWIID)](https://fsolt.org/swiid/){target="_blank"} can be downloaded as a .csv file from [Github](https://github.com/fsolt/swiid){target="_blank"}.
