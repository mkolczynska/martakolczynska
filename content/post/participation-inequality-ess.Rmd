...
categories: ["R"]
tags: ["surveys", "ESS", "R", "political inequality", "political participation", "cross-national research"]
output: 
  blogdown::html_page:
    toc: true
    toc_depth: 4
...
How to measure political inequality? The [Variaties of Democracy project (V-Dem)](https://www.v-dem.net/en/) has a set of political equality indicators that capture the extent to which political power is distributed according to wealth and income, membership in a particular social group, gender or sexual orientation (cf. [V-Dem Codebook v.7.1](https://www.v-dem.net/media/filer_public/84/a8/84a880ae-e0ca-4ad3-aff8-556abfdaff70/v-dem_codebook_v71.pdf), p. 259-262). The level of political equality is scored on an ordinal scale for each indicator by experts and then converted to an interval variable based on a measurement model.

In this post I try to construct a measure of the inequality of political participation, which can be considered a part of political inequality, derived directly from individual-level survey reported participation in political activities. I use data from the [European Social Survey, round 8](https://www.europeansocialsurvey.org/data/download.html?r=8) *via* the [`essurvey` package](https://github.com/ropensci/essurvey). I then check how this measure of inequality of political participation covaries with the [V-Dem](https://www.v-dem.net/en/about/) Political Equality indicator, economic inequality (from [SWIID](https://fsolt.org/swiid/)), and the [Worldwide Governance Indicators](http://info.worldbank.org/governance/wgi/).

The code for manipulating the data and plotting can be found [here](https://github.com/mkolczynska/martakolczynska/blob/master/content/post/participation-inequality-ess.Rmd).
...
alldata <- read.csv("C:/Users/mkolc/Google Drive/Academia/Blog/drafts/ess8-democr-politineq.csv")
...
After obtaining the three variants of the Political Participation Score for each respondent, I calculate their country means. Mapping the country means of the unweighted Political Participation Score shows a familiar pattern: Scandinavia has the highest mean value of participation, followed by Western Europe, with Eastern Europe at the bottom of the list.

```{r ineq-map, warning=FALSE, message=FALSE, echo = FALSE, fig.width = 9, fig.height = 5}
...
ggplot(merge, aes(fill=sum.pol.part.level)) +
  geom_sf(alpha=0.8,col='white') +
  coord_sf(crs="+proj=aea +lat_1=36.333333333333336 +lat_2=65.66666666666667 +lon_0=14") +
  ggtitle("Mean Value of the Unweighted Political Participation Score") + 
  labs(caption="Source: European Social Survey Round 8") +
...
Now I construct a measure of inequality for each variant of PPS by country. There are different measures of inequality, but for this exercise I will only use the Gini coefficient

The dot plot shows mean values and Gini indices of the three variants of the Political Participation Score. The mean of political participation is highest in Iceland and its inequality is lowest in Iceland (except for the unweighted PPS, by a small margin). Hungary, Russia and Lithuania have the lowest means of political participation, and are at the top with regard to participation inequality. The graph also shows that while both versions of PPS maintain the rank order, weighting increases differences in mean levels and inequality.
...
The V-Dem dataset (downloadable via the [`vdem` package](https://github.com/xmarquez/vdem)) aside its own democracy indicators also includes external variables, such as [Freedom House ratings](https://freedomhouse.org/) and the [Worldwide Governance Indicators (WGI)](http://info.worldbank.org/governance/wgi/#home). Since the ESS data cover Europe (and Israel) only, there won't be enough variation in the Freedom House ratings. I use WGI estimates, and the V-Dem Participatory Democracy Index, which measures the extent to which "the ideal of participatory democracy [is] achieved" ([V-Dem Codebook v.7.1](https://www.v-dem.net/media/filer_public/84/a8/84a880ae-e0ca-4ad3-aff8-556abfdaff70/v-dem_codebook_v71.pdf), p. 52). I select indicators from 2015, one year before the ESS Round 8 started.
...
Political inequality and its links to economic inequality are the focus of the grant I currently work in, titled “Political Voice and Economic Inequality across Nations and Time”, led by dr. Joshua Dubrow at the [Institute of Philosophy and Sociology, Polish Academy of Sciences](http://www.ifispan.pl/en/), and funded by Poland's [National Science Centre](https://ncn.gov.pl/?language=en). More information about this project can be found [here](https://politicalinequality.org/2017/09/21/new-project-political-voice-and-economic-inequality-across-nations-and-time/) and [here](https://ncn.gov.pl/sites/default/files/listy-rankingowe/2016-09-15/streszczenia/356936-en.pdf).
